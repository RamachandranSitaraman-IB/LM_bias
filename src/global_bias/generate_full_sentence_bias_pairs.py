...
# https://github.com/MilaNLProc/honest/blob/main/resources/binary/en_template.tsv
import numpy as np
import torch
...
from global_bias_utils import postprocess_next_token_scores
            ...
            for i in range(nums_iter):
                ...
                for context in basic_context:
                    ...
                    while cur_len < max_len:
                        ...
                        outputs_P = model.transformer(input_ids=input_ids)[0][:,
                                    -1].cpu().detach().numpy()  # transformer output: (2, batch, len, dim), output_P: (batch, dim)
                        outputs_P = np.multiply(np.array([1 - ratio[ii] for ii in range(batch_size)]).reshape(-1, 1),
                                                outputs_P.dot(P)) + \
                                    np.multiply(np.array([ratio[ii] for ii in range(batch_size)]).reshape(-1, 1),
                                                outputs_P)
                        ...
                        if do_sample:
                            ...
                            next_token_logscores = top_k_top_p_filtering(scores, top_k=top_k,
                                                                         top_p=top_p)  # batch * vocab
                            # Sample
                        ...
                        resultslast.append(activations)
                        resultsfirst.append(activations0)
                        layernamelast.append(layer_name)
                        layernamefirst.append(layer_name0)
                    ...
                    losses = F.nll_loss(logits.reshape(-1, logits.shape[-1]),
                                        input_ids[:, -logits.shape[1]:].reshape(-1).to(logits.device), reduction='none')
                    ...
                    for ii in range(batch_size):
                        ...
                        print(ii, gen_sent, " at ratio", A[a])
...
if __name__ == '__main__':
    ...
    model_class, tokenizer_class = MODEL_CLASSES["openai-gpt"]
